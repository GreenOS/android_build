...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^greenos_") ; then
       GREENOS_BUILD=$(echo -n $1 | sed -e 's/^greenos_//g')
       export BUILD_NUMBER=$((date +%s%N ; echo $GREENOS_BUILD; hostname) | openssl sha1 | sed -e 's/.*=//g; s/ //g' | cut -c1-10)
    else
       GREENOS_BUILD=
    fi
    export GREENOS_BUILD
...
function print_lunch_menu()
{
    ...
    echo
    if [ "z${GREENOS_DEVICES_ONLY}" != "z" ]; then
...
function breakfast()
{
    target=$1
    local variant=$2
    GREENOS_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/greenos/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the GREENOS model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi
            lunch greenos_$target-$variant
        fi
    fi
    return $?
}
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the GREENOS github
        T=$(gettop)
        pushd $T > /dev/null
...
{
    if [ "$OUT" ] ; then
        MODVERSION=$(get_build_var GREENOS_VERSION)
        ZIPFILE=greenos-$MODVERSION.zip
        ZIPPATH=$OUT/$ZIPFILE
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
    ...
    if (adb shell cat /system/build.prop | grep -q "ro.greenos.device=$GREENOS_BUILD");
    then
...
EOF
    ...
    return $?
        ...
        echo "The connected device does not appear to be $GREENOS_BUILD, run away!"
    fi
}
...
# Add completions
check_bash_version && {
    dirs="sdk/bash_completion vendor/greenos/bash_completion"
    for dir in $dirs; do
    if [ -d ${dir} ]; then
